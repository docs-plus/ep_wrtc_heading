...
const q = new Queue(function ({socket, padId, userData, target, callback, retry}, cb) {
	...
	if(_.get(roomStatus, `${padId}.${userData.headerId}.acceptCall`,false)) {
		...
		if(retry < maxTryToWaitAcceptNewCall)
			q.push({socket, padId, userData, target, callback, retry: retry+=1})
		else 
			callback(null, null, target)// acceptNewConnection({socket, padId, userData, target, callback, retry})
	}
