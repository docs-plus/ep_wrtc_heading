...
const q = new Queue(((args, cb) => {
  ...
  if (_.get(roomStatus, `${padId}.${userData.headerId}.acceptCall`, false)) {
    ...
    if (retry < maxTryToWaitAcceptNewCall) {
      q.push({
        socket, participators, padId, userData, target, callback, retry: retry + 1,
      });
      ...
      acceptNewConnection({
        socket, participators, padId, userData, target, callback, retry,
      });
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('acceptNewCall', (padId, headerId, callback) => {
      if (!_.get(roomStatus, `${padId}.${headerId}.acceptCall`, false)) {
        ...
        console.info(`yup, room ready to accept a new call. ${padId}.${headerId}`);
      }
    });
