	...
	var WRTC_Room = {
		...
		removeUserFromRoom: function (data) {
			...
			if (data.userId === clientVars.userId) {
				...
				$headingRoom
					.find(".wbrtc_roomBoxFooter button")
					.html("<b></b>(</span class='userCoutn'>" + userCount + "</span>)")
					.attr({
		...
		bulkUpdateRooms: function (hTagList) {
			var _self = this
		...
		activeEventListenr: function () {
			...
			_self.$body_ace_outer().on("mouseover", ".wbrtc_roomBox", function () {
				$(this)
					.addClass("active")
					.find(".wbrtc_roomBoxFooter, .wbrtc_roomBoxBody, .wbrtc_roomBoxHeader b")
					.show()
			})
