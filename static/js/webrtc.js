...
window.WRTC = (function () {
	...
	var self = {
		...
		aceSetAuthorStyle: function (hook, context) {
			if (context.author) {
				var user = self.getUserFromId(context.author)
				if (user) {
					$("#video_" + context.author.replace(/\./g, "_"))
						.css({
		...
		userJoinOrUpdate: function (hook, context) {
			/*
      ...
      */
		...
		handleClientMessage_RTC_MESSAGE: function (hook, context) {
		...
		showUserMediaError: function (err) {
			...
			switch (err.name) {
				case "CustomNotSupportedError":
					reason =
						"Sorry, your browser does not support WebRTC. (or you have it disabled in your settings).<br><br>" +
		...
		hide: function (userId) {
			...
			$("#rtcbox")
				.find("#video_a_" + userId)
				.parent()
				.remove()
		},
		activate: function (headingId) {
			self.show()
			self.hangupAll()
			self.getUserMedia(headingId)
		},
		deactivate: function (userId, headingId) {
			self.hide(userId)
      ...
      */
		...
		hangupAll: function (hangupAll) {
			Object.keys(pc).forEach(function (userId) {
				self.hangup(userId, true, hangupAll)
			})
		},
		getUserId: function () {
			return self._pad && self._pad.getUserId()
		},
		hangup: function (userId, notify, headingId) {
		...
		call: function (userId, headingId) {
			if (!localStream) {
		...
		createPeerConnection: function (userId, headingId) {
			if (pc[userId]) {
		...
		getUserMedia: function (headingId) {
