(function (f) { if (typeof exports === 'object' && typeof module !== 'undefined') { module.exports = f(); } else if (typeof define === 'function' && define.amd) { define([], f); } else { let g; if (typeof window !== 'undefined') { g = window; } else if (typeof global !== 'undefined') { g = global; } else if (typeof self !== 'undefined') { g = self; } else { g = this; }g.adapter = f(); } })(() => {
   ...
   */
    ...
    const sdp = _interopRequireWildcard(_sdp);
   ...
   */
    ...
    function shimParameterlessSetLocalDescription(window, browserDetails) {
      ...
      window.RTCPeerConnection.prototype.setLocalDescription = function setLocalDescription() {
        ...
        return func.apply(this).then((d) => nativeSetLocalDescription.apply(_this, [d]));
      };
    }
   ...
   */
    ...
    // https://bugzilla.mozilla.org/show_bug.cgi?id=1396918
      ...
      window.RTCPeerConnection.prototype.createOffer = function createOffer() {
        ...
        if (this.setParametersPromises && this.setParametersPromises.length) {
          return Promise.all(this.setParametersPromises).then(() => origCreateOffer.apply(_this4, _arguments2))
            .finally(() => {
    ...
    // https://bugzilla.mozilla.org/show_bug.cgi?id=1396918
      ...
      window.RTCPeerConnection.prototype.createAnswer = function createAnswer() {
        ...
        if (this.setParametersPromises && this.setParametersPromises.length) {
          return Promise.all(this.setParametersPromises).then(() => origCreateAnswer.apply(_this5, _arguments3))
            .finally(() => {
  ...
  /* eslint-env node */
    ...
    SDPUtils.parseRtpEncodingParameters = function (mediaSection) {
      ...
      const ssrcs = SDPUtils.matchPrefix(mediaSection, 'a=ssrc:').map((line) => SDPUtils.parseSsrcMedia(line))
        .filter((parts) => parts.attribute === 'cname');
      const primarySsrc = ssrcs.length > 0 && ssrcs[0].ssrc;
      let secondarySsrc = void 0;
    ...
    SDPUtils.parseRtcpParameters = function (mediaSection) {
      ...
      // SSRCs.
      const remoteSsrc = SDPUtils.matchPrefix(mediaSection, 'a=ssrc:').map((line) => SDPUtils.parseSsrcMedia(line))
    ...
    SDPUtils.parseMsid = function (mediaSection) {
      ...
      const planB = SDPUtils.matchPrefix(mediaSection, 'a=ssrc:').map((line) => SDPUtils.parseSsrcMedia(line))
